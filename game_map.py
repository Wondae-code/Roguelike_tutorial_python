...
class GameMap:
    def __init__(self, engine: Engine, width: int, height: int, entities: Iterable[Entity] = ()):
        ...
        self.explored = np.full(
            (width, height), fill_value=False, order="F")  # 플레이어가 봤었던 타일
...
class GameWorld:
    """
        게임맵의 세팅을 유지, 다음 맵으로 넘어갈 때, 새로운 맵을 생성
    """

    def __init__(
            self, *, engine: Engine, map_width: int, map_height: int, max_rooms: int,
            room_min_size=int, room_max_size=int, max_monsters_per_room=int, max_items_per_room: int, current_floor: int = 0):
        self.engine = engine
